...
DBT_PROJECT_NAME = os.getenv('DBT_PROJECT_NAME', 'dbt_BNPL')
DBT_PROJECT_PATH = (
...
with DAG(
    ...
    for entity_key, config in ENTITIES.items():
        ...
        load_task = SQLExecuteQueryOperator(
            ...
            sql=f"""
                COPY INTO RAW.BNPL.{table}
                FROM @RAW.BNPL.my_s3_stage_direct/{folder_path}
                FILE_FORMAT = (FORMAT_NAME = RAW.BNPL.csv_format_for_load)
                PATTERN = '.*\\.csv';
            """
        )
